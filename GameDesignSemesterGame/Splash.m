...
-(id) init {
    if( (self=[super init])) {
        ...
        CCSprite* logo = [CCSprite spriteWithFile: @"robo1.gif"];
        
        [logo setPosition:ccp(screenWidth/2,screenHeight*0.65)];
        
        [self addChild: logo];
        
        CCMenuItem *goButton = [CCMenuItemFont itemWithString:@"Kickpuncher The Game" target:self selector:@selector(onGo:)];
