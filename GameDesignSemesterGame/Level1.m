...
- (id) init {
    if([self isMemberOfClass:[Level1 class]])
        world = [CCTMXTiledMap tiledMapWithTMXFile:@"kickpuncher_level1.tmx"];
...
- (void) initWorld {
    ...
    stalksLayer = [world layerNamed:@"ladders"];
...
//	[self unschedule:@selector(caughtReset)];
    ...
    [grace reset];
    [self setPosition:ccp(TOPOINT(grace.x-96.0),TOPOINT(grace.y-64.0))];
